		...
		<div className="flex flex-col h-screen bg-gray-100">
			<header className="bg-primary shadow-md px-4 py-3 sticky top-0 z-40 transition-all duration-300">
				...
				>
					...
					/>
					<span className="font-bold text-lg text-white">Admin Cabang</span>
				</div>

				{/* Navigasi (akan tetap ada) */}
				<nav className="flex items-center justify-between gap-2 sm:gap-4">
					<NavItem to="/absensi" icon={FiClipboard} label="Absensi" />
					<NavItem to="/jadwal" icon={FiCalendar} label="Jadwal" />
					<NavItem to="/laporan" icon={FiFileText} label="Laporan" />
					<NavItem to="/members" icon={FiUsers} label="Member" />
					<NavItem to="/cabang/settings" icon={FiSettings} label="Profile" />
				</nav>
			</header>
